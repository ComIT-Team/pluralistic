...
const Cart = () => {
  ...
  return (
    <div className="w-full bg-gray-100 py-4 ">
      {
        products.length > 0 ?(
            <div className="container mx-auto h-auto grid grid-cols-2  md:grid-cols-3  lgl:gap-8 bg-opacity-20">
       <div className="w-full h-full bg-white px-4 col-span-2 md:col-span-4 ">
        <div className="font-titleFont flex items-center justify-between border-b-[1px] border-b-gray-400 py-3 text-black">
            <h2 className="text-3xl font-medium">Shopping Cart</h2>
            <h3 className="text-xl font-normal">Price</h3>
        </div>
        {/* ======Products Display====== */}
        <div>
        {
            products.map((item)=>(
                ...
                <div key={item.id} className="w-full border-b-[1px] border-b-gray-300 p-4 flex items-center gap-6 -ml-10">
                <div className="w-1/5">
                    <img className="w-full h-44 object-contain" src= {item.image} alt="img"/>
                </div>
                <div className="text-black w-2/4 md:w-4/5">
                    <h2 className="font-semibold text-lg ">{item.title}</h2>
                    <p className="pr-10 text-sm">{item.description.substring(0,80)}</p>
        ...
        className="px-10 py-2 bg-red-500 hover:bg-red-600 active:bg-red-500 text-white rounded-lg font-titleFont font-semibold text-lg tracking-wide my-1 mb-6 sm:mx-[88px]">
        Clear Cart
        </button>
        </div>
       </div>
        <div className="w-full h-52 bg-white col-span-1 text-black flex flex-col items-center justify-center p-4 sm:mx-[88px] sm:mt-2">
        <div>
